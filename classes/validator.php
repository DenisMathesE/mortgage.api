...
class Validator {
    ...
    protected function getBorrowerValidate( $data ) {
         return [
             ...
             'marriage_status' => ['required' => (($data['type'] == 1)?1:0), 'preg_replace' => ['/[^\d]/', ''], 'minlength' => 1, 'error_message' => 'Семейное положение'],
    ...
    protected function getAddOrderValidate() {
        return [
            'passport' => [
                'required' => true,
                'type' => 'array',
         ...
         if ( !isset($data[$name]) && isset($validateItem['required']) && $validateItem['required'] ) {
            ...
            if ( isset($validateItem['childs']) && count($validateItem['childs']) ) {
                foreach ( $validateItem['childs'] as $nameChild => $validateItemChild ) {
                    $data[$name][$nameChild] = $this->checkValidateItem( $nameChild, $validateItemChild, $data[$name], (($error_name)?$error_name.".":'').$nameChild );
                }
            } else {
